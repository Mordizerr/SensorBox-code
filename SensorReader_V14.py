...
# ============================================================================
# FEATURE 6: BACKGROUND CSV LOGGER + ROLLING BACKUP SYSTEM
# ============================================================================
...
# ============================================================================
# ROLLING BACKUP SYSTEM (Power Loss Protection)
# ============================================================================

class RollingBackupLogger:
    """Maintain rolling 10-slot backup system for power loss protection.

    Each experiment session claims one slot at start time and overwrites that
    same file on every subsequent periodic backup, leaving all other slots
    untouched.  When a new session starts and all 10 slots are occupied the
    oldest file is deleted first to free a slot.
    """
    ...
    def __init__(self, config):
        self.config = config
        self.backup_directory = "sensor_backups"
        os.makedirs(self.backup_directory, exist_ok=True)
        self._session_file = None   # fixed path for the current session

    def claim_slot(self):
        """Claim one backup slot for a new session.

        Deletes the oldest existing backup if all slots are full, then
        records the path of the new slot file (not yet written to disk).
        """
        try:
            backup_files = sorted([
            ...
            if len(backup_files) >= self.MAX_BACKUPS:
                oldest = os.path.join(self.backup_directory, backup_files[0])
                os.remove(oldest)

            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self._session_file = os.path.join(
                self.backup_directory, f"backup_{timestamp}.csv"
            )
        except Exception as e:
            print(f"Backup slot claim error: {e}")

    def release_slot(self):
        """Forget the current session slot (does not delete the file)."""
        self._session_file = None

    def create_backup(self, data_list):
        """Overwrite the current session's backup slot with latest data."""
        if not data_list or not self._session_file:
            return
...
class BackupWorker(QThread):
    """Run backup file writes in a background thread to avoid blocking the UI."""

    def __init__(self, backup_logger, data_snapshot):
        super().__init__()
        self.backup_logger = backup_logger
        # Work on a snapshot so the main thread can keep appending to collected_data
        self.data_snapshot = data_snapshot

    def run(self):
        self.backup_logger.create_backup(self.data_snapshot)
...
class StatisticsWidget(QGroupBox):
    """Display statistics using incremental O(1) computation (Welford's algorithm)"""
    
    def __init__(self):
        super().__init__("Statistics")
        self.initUI()
        
        # Bounded deques for history (used only for get_statistics_dict / reports)
        self.ec_history = deque(maxlen=500)
        self.temp_history = deque(maxlen=500)
        self.ph_history = deque(maxlen=500)

        # Incremental accumulators — updated in O(1) per measurement
        self._stats = {
            'ec':   self._make_acc(),
            'temp': self._make_acc(),
            'ph':   self._make_acc(),
    ...
    def _make_acc(self):
        """Return a fresh Welford accumulator dict."""
        return {'n': 0, 'mean': 0.0, 'M2': 0.0, 'min': float('inf'), 'max': float('-inf')}

    def _update_acc(self, acc, value):
        """Update Welford accumulator with one new value (O(1))."""
        acc['n'] += 1
    ...
    def _acc_std(self, acc):
        """Population standard deviation from accumulator."""
        if acc['n'] < 2:
            return 0.0
        return (acc['M2'] / acc['n']) ** 0.5
    ...
    def update_statistics(self, ec, temp, ph):
        """Update statistics — O(1) per call regardless of history length."""
        self.ec_history.append(ec)
        self.temp_history.append(temp)
        self.ph_history.append(ph)

        self._update_acc(self._stats['ec'],   ec)
        self._update_acc(self._stats['temp'], temp)
        self._update_acc(self._stats['ph'],   ph)

        self._update_row(0, ec,   self._stats['ec'],   "µS/cm")
        self._update_row(1, temp, self._stats['temp'], "°C")
        self._update_row(2, ph,   self._stats['ph'],   "")
        
    def _update_row(self, row, current, acc, unit):
        """Update table row from pre-computed accumulator values."""
        if acc['n'] == 0:
            return

        self.stats_table.setItem(row, 1, QTableWidgetItem(f"{current:.2f} {unit}"))
        self.stats_table.setItem(row, 2, QTableWidgetItem(f"{acc['mean']:.2f} {unit}"))
        self.stats_table.setItem(row, 3, QTableWidgetItem(f"{self._acc_std(acc):.2f} {unit}"))
        self.stats_table.setItem(row, 4, QTableWidgetItem(f"{acc['min']:.2f} {unit}"))
        self.stats_table.setItem(row, 5, QTableWidgetItem(f"{acc['max']:.2f} {unit}"))
        
    def get_statistics_dict(self):
        """Get statistics as dictionary for reports."""
        stats = {}
        if self._stats['ec']['n'] > 0:
            ec_acc   = self._stats['ec']
            temp_acc = self._stats['temp']
            ph_acc   = self._stats['ph']
            stats = {
                'count':     ec_acc['n'],
                'duration':  self.ec_history[-1] if self.ec_history else 0,
                'ec_mean':   ec_acc['mean'],
                'ec_std':    self._acc_std(ec_acc),
                'ec_min':    ec_acc['min'],
                'ec_max':    ec_acc['max'],
                'temp_mean': temp_acc['mean'],
                'temp_std':  self._acc_std(temp_acc),
                'temp_min':  temp_acc['min'],
                'temp_max':  temp_acc['max'],
                'ph_mean':   ph_acc['mean'],
                'ph_std':    self._acc_std(ph_acc),
                'ph_min':    ph_acc['min'],
                'ph_max':    ph_acc['max'],
            }
        return stats
...
class SensorReaderSegment1(QMainWindow):
    def __init__(self):
        ...
        self.backup_logger = RollingBackupLogger(self.config)  # NEW: Rolling backup system
        ...
        # NEW: Periodic backup timer (every 60 seconds)
        self.backup_timer = QTimer()
        self.backup_timer.timeout.connect(self.periodic_backup)
        self.backup_interval_ms = 60000  # 60 seconds
        
        self.start_time = time.time()
        
        self.setupUI()
        
        # Backup system ready — slot is claimed when measurements start
        self.log("🔒 Session backup ready (10-slot rolling system, 1 slot per experiment)")
    ...
    def create_toolbar(self):
        ...
        # NEW: Backup status info
        backup_info = QAction("🔒 Backups Active (10-slot rolling, 1 per experiment)", self)
        backup_info.setEnabled(False)  # Display only, not clickable
        toolbar.addAction(backup_info)
        
        # Create spacer to push restart button to the right
        spacer = QWidget()
        spacer.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        toolbar.addWidget(spacer)
        
        # NEW: Restart button (top right)
        restart_action = QAction("🔄 Restart", self)
        restart_action.triggered.connect(self.restart_application)
        restart_action.setStatusTip("Restart all processes (connection, data, graphs)")
    ...
    def toggle_measurements(self):
        ...
        if self.measurement_timer.isActive():
            ...
            self.create_session_backup()  # Final overwrite of this session's slot
            self.backup_logger.release_slot()  # Done with this slot
            self.log("🔒 Final backup saved")
        else:
            # Claim one backup slot for this session before starting
            self.backup_logger.claim_slot()
            # Always reset averaging buffer on fresh start so no leftover
            # readings from a previous run contaminate the first averaged point
            self._avg_buffer = []
            self._avg_count = self.avg_spin.value()
            interval = self.interval_spin.value() * 1000
            self.measurement_timer.start(interval)
            self.backup_timer.start(self.backup_interval_ms)
            ...
            self.log("🔒 Periodic backup active (every 60 seconds, 1 slot per session)")
    ...
    def log(self, message):
        """Log message — keeps only the last 1000 lines to prevent UI slowdown"""
        timestamp = datetime.now().strftime("%H:%M:%S")
        self.console.append(f"[{timestamp}] {message}")

        # Trim oldest lines when the document grows too large
    ...
    def create_session_backup(self):
        """Create a backup CSV for the current session (runs in background thread)"""
        if not self.collected_data:
            return
        worker = BackupWorker(self.backup_logger, list(self.collected_data))
        worker.start()
        # Keep a reference so Python doesn't garbage-collect the thread mid-run
        self._backup_workers = getattr(self, '_backup_workers', [])
        self._backup_workers.append(worker)
        # Prune finished workers to avoid unbounded list growth
        self._backup_workers = [w for w in self._backup_workers if w.isRunning()]

    def periodic_backup(self):
        """Periodic backup every 60 seconds during measurements (non-blocking)"""
        if len(self.collected_data) > 0:
            self.create_session_backup()
            self.log(f"🔒 Backup queued: {len(self.collected_data)} data points")
        
    def restart_application(self):
        """Restart all processes without restarting GUI"""
        # Confirm with user
        reply = QMessageBox.question(
            self, "Restart Processes",
            "This will reset:\n"
            "• Connection status\n"
            "• All cached measurements\n"
        ...
        self._uncalibrated_warning_shown = False
        self._reading_buffer = None
        ...
        for row in range(3):  # 3 rows: EC, Temp, pH
            for col in range(6):  # 6 columns
                self.stats_widget.stats_table.setItem(row, col, QTableWidgetItem(""))
        
        # Clear history and reset incremental accumulators
        self.stats_widget.ec_history.clear()
        self.stats_widget.temp_history.clear()
        self.stats_widget.ph_history.clear()
        self.stats_widget._stats = {
            'ec':   self.stats_widget._make_acc(),
            'temp': self.stats_widget._make_acc(),
            'ph':   self.stats_widget._make_acc(),
        }
        
        # Reset labels
        self.ec_label.setText("EC: ---")
        ...
        # Stop logging if active
        if self.background_logger.is_logging:
            self.background_logger.stop_logging()
            self.log_btn.setText("Start Logging")
            self.log_btn.setStyleSheet("background-color: #339af0; color: white; font-weight: bold;")

        # Release the backup slot so next session starts clean
        self.backup_logger.release_slot()

        self.log("=" * 60)
        self.log("🔄 SYSTEM RESTART - All processes reset")
        self.log("=" * 60)
    ...
    def closeEvent(self, event):
        ...
        # Save final backup synchronously before closing — we're shutting down
        # so a brief block is fine and guarantees the write completes before exit.
        if self.collected_data and self.backup_logger._session_file:
            self.backup_logger.create_backup(list(self.collected_data))
        self.backup_logger.release_slot()
        self.log("🔒 Final session backup saved on close")
